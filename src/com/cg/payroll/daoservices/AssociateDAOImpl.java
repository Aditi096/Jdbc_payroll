...
public class AssociateDAOImpl implements AssociateDAO {
	...
	public Associate save(Associate associate) {
		try {
			...
			pstmt1.setString(5, associate.getDesignaton());
	...
	public boolean update(Associate associate) {
		 try {
	            con.setAutoCommit(false);
	            PreparedStatement  pstmt1 = con.prepareStatement("UPDATE salary set netSalary="+associate.getSalary().getNetSalary()+"conveyenceAllowance="+associate.getSalary().getConveyenceAllowance()+"hra="+associate.getSalary().getHra()+"otherAllowance="+associate.getSalary().getOtherAllowance()+"personalAllowance="+associate.getSalary().getPersonalAllowance()+"maonthlyTax="+associate.getSalary().getMonthlyTax()+"grossSalary="+associate.getSalary().getGrossSalary()+"where associateId="+associate.getAssociateId());
	            pstmt1.executeUpdate();
	            con.commit();
	...
	public List<Associate> findAll() {
		ArrayList associates=new ArrayList<Associate>();

		try {
			PreparedStatement pstmt1=con.prepareStatement("select * form Associate");
